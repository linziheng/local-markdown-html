...
5. Select **+** to create a note. The first time you select **Save** for that note, choose its folder. Later changes save automatically.

Local Markdown asks Chrome for access only to the folder you select. Your Markdown files and pasted images remain local.

## Pasting images

Paste an image into a note. If that note has not been saved yet, Local Markdown asks you to choose its folder once. It saves the Markdown file and image in that folder, then inserts a relative Markdown image link. Filenames begin with a timestamp so earlier images sort before later images.
